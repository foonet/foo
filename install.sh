...
RHOST='download.foonet.org'
CHOST='download.foonet.org'
VERSION='0.9.8'
software="nginx apache2 apache2-utils apache2.2-common bsdutils e2fsprogs
    ...
    libapache2-mod-php5 mc exim4 exim4-daemon-heavy clamav-daemon
    ...
    fail2ban dnsutils foo foo-nginx foo-php vsftpd"
...
wget -q "$CHOST/$VERSION/foo.conf" -O /dev/null
if [ $? -ne 0 ]; then
...
for pkg in exim4 mysql-server apache2 nginx foo-nginx foo-php vsftpd; do
...
if [ "$memory" -lt '350000' ] && [ -z "$force" ]; then
    echo "Error: not enough memory to install Foo Control Panel."
...
# Install foo repo
echo "deb http://$RHOST/$VERSION/ /" > $apt/foo.list
wget $CHOST/$VERSION/foo_sign.key -O foo_sign.key
apt-key add foo_sign.key
...
# Backup foo
service foo stop > /dev/null 2>&1
if [ -e '/usr/local/foo' ]; then
    cp -r /usr/local/foo/* $vst_backups/foo/
    apt-get -y remove foo foo-nginx foo-php vsftpd
    apt-get -y purge foo foo-nginx foo-php vsftpd
    rm -rf /usr/local/foo
    rm -rf /root/foo_sign.key
echo OK
fi
...
apt-get -y install $software --force-yes
if [ $? -ne 0 ]; then
...
# Foo configuration
echo "export FOO='/usr/local/foo'" > /etc/profile.d/foo.sh
chmod 755 /etc/profile.d/foo.sh
source /etc/profile.d/foo.sh
echo 'PATH=$PATH:/usr/local/foo/bin' >> /root/.bash_profile
echo 'export PATH' >> /root/.bash_profile
source /root/.bash_profile
wget $CHOST/$VERSION/foo.log -O /etc/logrotate.d/foo
...
mkdir -p $FOO/ssl
mkdir -p $FOO/data
mkdir -p $FOO/data/ips
mkdir -p $FOO/data/queue
mkdir -p $FOO/data/users
mkdir -p $FOO/data/firewall
touch $FOO/data/queue/backup.pipe
touch $FOO/data/queue/disk.pipe
touch $FOO/data/queue/webstats.pipe
touch $FOO/data/queue/restart.pipe
touch $FOO/data/queue/traffic.pipe
chmod 750 $FOO/conf
chmod 750 $FOO/data/users
chmod 750 $FOO/data/ips
chmod -R 750 $FOO/data/queue
ln -s /usr/local/foo/log /var/log/foo
touch /var/log/foo/system.log
touch /var/log/foo/nginx-error.log
touch /var/log/foo/auth.log
chmod 660 /var/log/foo/*
...
# foo.conf
wget $CHOST/$VERSION/foo.conf -O $FOO/conf/foo.conf
if [ "$srv_type" = 'micro' ] ||  [ "$srv_type" = 'small' ]; then
    sed -i "s/clamav-daemon//g" $FOO/conf/foo.conf
    sed -i "s/spamassassin//g" $FOO/conf/foo.conf
fi
...
/usr/local/foo/bin/foo-change-sys-hostname $servername 2>/dev/null

# Templates
cd /usr/local/foo/data
...
if [ "$disable_iptables" = 'yes' ]; then
    sed -i "s/iptables//" $FOO/conf/foo.conf
else
    $FOO/bin/foo-update-firewall
fi

# Generating SSL certificate
$FOO/bin/foo-generate-ssl-cert $(hostname) $email 'US' 'California' \
     'San Francisco' 'Foo Control Panel' 'IT' > /tmp/vst.pem
...
touch /etc/nginx/conf.d/foo.conf
...
wget $CHOST/$VERSION/apache2-status.conf -O /etc/apache2/mods-enabled/status.conf
wget $CHOST/$VERSION/apache2.log -O /etc/logrotate.d/apache2
rm -f /etc/apache2/conf.d/foo.conf
echo > /etc/apache2/conf.d/foo.conf
echo "# Powered by foo" > /etc/apache2/sites-available/default
echo "# Powered by foo" > /etc/apache2/sites-available/default-ssl
echo "# Powered by foo" > /etc/apache2/ports.conf
...
if [ -z "$disable_fail2ban" ]; then
    ...
    sed -i "s/fail2ban//" $FOO/conf/fooc.conf
fi
# php configuration
...
wget $CHOST/$VERSION/roundcube-driver.php -O \
    /usr/share/roundcube/plugins/password/drivers/foo.php
...
$FOO/bin/foo-add-user admin $vpass $email default System Administrator
if [ $? -ne 0 ]; then
    echo "Error: can't create admin user"
    exit 1
fi
$FOO/bin/foo-change-user-shell admin bash
$FOO/bin/foo-change-user-language admin en

# Configure mysql host
$FOO/bin/foo-add-database-host mysql localhost root $mpass
$FOO/bin/foo-add-database admin default default $(gen_pass) mysql

# Configuring system ips
$FOO/bin/foo-update-sys-ip
...
# Get remote ip
vst_ip=$(wget download.foonet.org/$VERSION/ip.php -O - 2>/dev/null)
if [ ! -z "$vst_ip" ] && [ "$vst_ip" != "$main_ip" ]; then
    # Set NAT association
    $FOO/bin/foo-change-sys-ip-nat $main_ip $vst_ip
fi
if [ -z "$vst_ip" ]; then
    vst_ip=$main_ip
fi
...
$FOO/bin/foo-add-web-domain admin foonet.org $vst_ip

# Add default dns domain
$FOO/bin/foo-add-dns-domain admin foonet.org $vst_ip

# Add default mail domain
$FOO/bin/foo-add-mail-domain admin foonet.org

# Configuring cron jobs
command='sudo /usr/local/foo/bin/foo-update-sys-queue disk'
$FOO/bin/foo-add-cron-job 'admin' '15' '02' '*' '*' '*' "$command"
command='sudo /usr/local/foo/bin/foo-update-sys-queue traffic'
$FOO/bin/foo-add-cron-job 'admin' '10' '00' '*' '*' '*' "$command"
command='sudo /usr/local/foo/bin/foo-update-sys-queue webstats'
$FOO/bin/foo-add-cron-job 'admin' '30' '03' '*' '*' '*' "$command"
command='sudo /usr/local/foo/bin/foo-update-sys-queue backup'
$FOO/bin/foo-add-cron-job 'admin' '*/5' '*' '*' '*' '*' "$command"
command='sudo /usr/local/foo/bin/foo-backup-users'
$FOO/bin/foo-add-cron-job 'admin' '10' '05' '*' '*' '*' "$command"
command='sudo /usr/local/foo/bin/foo-update-user-stats'
$FOO/bin/foo-add-cron-job 'admin' '20' '00' '*' '*' '*' "$command"
command='sudo /usr/local/foo/bin/foo-update-sys-rrd'
$FOO/bin/foo-add-cron-job 'admin' '*/5' '*' '*' '*' '*' "$command"

# Building inititall rrd images
$FOO/bin/foo-update-sys-rrd

# Enable file system quota
if [ "$quota" = 'yes' ]; then
    $FOO/bin/foo-add-sys-quota
fi
...
service foo start
if [ "$?" -ne 0 ]; then
    echo "Error: foo start failed"
    exit 1
fi

# Send notification to foonet.org
wget download.foonet.org/$VERSION/notify.php -O /dev/null

# Send notification to admin email
echo -e "Congratulations, you have just successfully installed \
the Foo Control Panel

You can login in Foo with following credentials:
    username: admin
    password: $vpass
    https://$vst_ip:8083

We hope that you enjoy your installation of Foo. Please \
...
send_mail="$FOO/web/inc/mail-wrapper.php"
cat $tmpfile | $send_mail -s "Foo Control Panel" $email
rm -f $tmpfile

# Congrats
echo
echo 'Congratulations,'
echo 'you have successfully installed Foo Control Panel.'
echo
echo '-------------------------------'
